...
$themes = array( "headerbg", "headertitlecolor", "navbarbg", "navbarlink", "mobilemenubg", "mobilelinkcolor" , "stickybar" );
for( $x=0; $x<sizeof( $themes ); $x++ ) {
	...
	if(  db_fetch_array($res) ) { 
		...
		$sql = "INSERT INTO `config` (`id`,`namespace`,`key`, `value` ) VALUES ((77779 + $x),'osticketawesome' , '" . $themes[$x] . "', '" . $_POST[ $themes[$x] ] . "')";
		$res = db_query($sql);
	}
